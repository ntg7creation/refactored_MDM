...
def collect_all_motions(root_dir):
    """
    Recursively collect all tpr_both.npy files under root_dir.
    """
    file_paths = []
    for dirpath, _, files in os.walk(root_dir):
        for f in files:
            if f == "tpr_both.npy" or f.endswith("tpr_both.npy"):
                file_paths.append(os.path.join(dirpath, f))
    return file_paths

def main():
    # Path to converted motions
    root_dir = r"D:\repos\refactored_MDM\GigaHands_Data\converted_tpr\hand_poses_tpr"
    out_dir = os.path.join(root_dir, "..", "norm_stats")
    os.makedirs(out_dir, exist_ok=True)

    file_paths = collect_all_motions(root_dir)
    assert len(file_paths) > 0, f"No tpr_both.npy found under {root_dir}"

    print(f"Found {len(file_paths)} tpr motion files")

    total_frames = 0
    sum_feats = None
    sum_sq_feats = None

    for path in tqdm(file_paths, desc="Accumulating TPR stats"):
        motion = np.load(path)  # expected shape [T, 120]
        if motion.ndim != 2:
    ...
    mean_path = os.path.join(out_dir, "mean_tpr_both.npy")
    std_path = os.path.join(out_dir, "std_tpr_both.npy")
